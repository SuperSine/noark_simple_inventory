...
class Item_Controller extends Base_Controller {
    ...
    public function get_mobile_index(){
        return View::make('layout.mobile')
                    ->nest('head','layout.blocks.head_mob')
                    ->nest('main','item.mobile_index');
    }

    public function get_mobile_assign(){
        return View::make('layout.mobile')
                    ->nest('head','layout.blocks.head_mob')
                    ->nest('main','item.mobile_assign');
    }   

    public function get_mobile_checkin(){
        
        return View::make('layout.mobile')
                    ->nest('head','layout.blocks.head_mob')
                    ->nest('main','item.mobile_checkout_1',
                            array(
                                'post_method' => action('item@checkin'),
                                'check_mode'  => 'Check In'
                            )
                        );
    ...
    public function get_mobile_checkout(){
        return View::make('layout.mobile')
                    ->nest('head','layout.blocks.head_mob')
                    ->nest('main','item.mobile_checkout_1',
                        array(
                            'post_method' => action('item@checkout'),
    ...
    public function get_list()
    {
        ...
        $list = DB::table('items')
                    ...
                    ->get(array('items.id','items.name','items.code','items.part_num','categories.name AS category','items.quantity','items.location'));
    ...
    public function post_add()
    {
        ...
        if(Input::get('submit'))
        {
            $rules = array(
                ...
                'part_num'      => 'required|max:20|code_unique',
    ...
    */
    ...
    private function fetch_document($column,$value,$data){
    ...
    private function insert_document($insert_array){
        $date = new \DateTime;
        ...
        $data_array = array_merge($insert_array,$self_array);

        $document = DB::table('documents')->insert_get_id($data_array);
